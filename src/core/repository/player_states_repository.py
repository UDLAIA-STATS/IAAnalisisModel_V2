...
class PlayerStatesRepository:
    ...
    def get_states_by_frame(match_id: int, frame_number: int, session: Session) -> Sequence[PlayerState]:
        query = (
            ...
            .where(PlayerState.frame_number == frame_number))
        results = session.exec(query).all()

        logfire.info(f"[PlayerStatesRepository] get_player_states_by_frame_num: {len(results)}")

        if results is None or len(results) == 0:
            logfire.error(f"PlayerStatesRepository.get_player_states_by_frame_num: No results found for frame number {frame_number} and match id {match_id} in database")
        return results
