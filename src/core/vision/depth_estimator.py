...
class DepthEstimator:
    ...
    def __init__(
        ...
        if depth_model_type not in DepthModelsTypes.to_dict().values():
            raise ValueError(f"Tipo de modelo de profundidad no reconocido: {depth_model_type}")
        ...
        self.frames_per_depth_calc = int(frame_rate * depth_calculation_interval_seconds)
        ...
        if depth_model_type == DepthModelsTypes.DPT_Large or depth_model_type == DepthModelsTypes.DPT_Hybrid:
    ...
    def _init_pose_landmarker(self):
        ...
        base_options = python.BaseOptions(
            model_asset_path=model_path.as_posix()
        )
    ...
    def should_calculate_depth(self, frame_num: int, current_camera_scale: float) -> bool:
        """
        ...
        scale_change = abs(current_camera_scale - self.last_camera_scale) / self.last_camera_scale if self.last_camera_scale != 0 else 0
    ...
    def process_player_depth(
        ...
        logfire.info(f"[DepthCalculator] Coordenadas extraidas del bbox: x1={x1}, y1={y1}, x2={x2}, y2={y2}")
        ...
        if waist_center_norm is None:
            logfire.debug(f"[DepthCalculator] Frame {frame_num}: No se detecto pose, usando centro del bbox")
            waist_x = (x1 + x2) / 2.0
            waist_y = (y1 + y2) / 2.0
        else:
    ...
    def _calculate_depth_at_point(self, frame: np.ndarray, x: int, y: int) -> float:
        ...
        logfire.debug(f"[DepthCalculator] Profundidad en ({x}, {y}): {depth_value}, tipo de valor {type(depth_value)}")
        logfire.debug(f"[DepthCalculator] Escala de profundidad: {self.depth_scale}, tipo de valor {type(self.depth_scale)}")
