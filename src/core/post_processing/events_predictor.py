# src/entities/analyzers/ball_possession_ml_analyzer.py
...
from pyspark.sql.functions import (
    avg, col, lit, when, row_number, count, sum as spark_sum,
    max as spark_max, min as spark_min, lag,
    sqrt, pow, acos, degrees, broadcast, rand, abs as spark_abs
)
...
from src.entities.predictors.ball_possession_analyzer_base import BallPossessionAnalyzerBase
...
class BallPossessionMLAnalyzer(BallPossessionAnalyzerBase):
    ...
    SHOT_PROB_THRESHOLD: float = 0.5

    def analyze(self, match_id: int, session: Session) -> None:
        logfire.info(f"[BallPossessionMLAnalyzer] Starting per‑match ML training for {match_id}")
        ...
        goal_centroid = self._compute_goal_centroid(goals_df) if not goals_df.isEmpty() else None
        if goal_centroid:
        ...
        # 6. Update database
        self._update_players(match_id, possession_times, shot_counts, goal_assignments, session)

        logfire.info(f"[BallPossessionMLAnalyzer] Analysis complete for match {match_id}")
    ...
    def _train_possession_model(self, df: DataFrame):
        ...
        if train_df.count() < 10:
            logfire.warning("Too few labelled frames for possession training; skipping ML.")
            return None
        ...
        rf = RandomForestClassifier(featuresCol="scaled_features", labelCol="has_ball",
    ...
    def _predict_possession(self, df: DataFrame, model: PipelineModel) -> DataFrame:
        ...
        # probability[1] is the probability of having the ball
        ...
        result = result.withColumn(
            "possession_pred",
            when(col("possession_prob") >= self.POSSESSION_PROB_THRESHOLD, 1).otherwise(0)
        )
        return result
    ...
    def _train_shot_model(self, df: DataFrame, goals_df: DataFrame):
        ...
        Create training data for shots:
          - Positive: frames where a goal occurred (or within a small window before it)
          - Negative: random frames where dist_ball_goal > threshold (far from goal)
        """
        # Positive: any frame within GOAL_ASSOCIATION_WINDOW before a goal
        # We'll join with goals on timestamp proximity (broadcast join)
        goals = goals_df.withColumnRenamed("timestamp", "goal_ts")
        pos_df = df.join(broadcast(goals), how="inner") \
                   .withColumn("time_diff", col("goal_ts") - col("timestamp")) \
                   .filter((col("time_diff") >= 0) & (col("time_diff") <= self.GOAL_ASSOCIATION_WINDOW)) \
                   .withColumn("label", lit(1)) \
                   .select("label", "dist_pb", "player_speed", "ball_speed",
                           "dist_ball_goal", "angle_ball_goal")

        # Negative: sample frames far from any goal (distance > threshold)
        # We'll take a random sample of non‑positive frames.
        # Use a rule: distance > 2*SHOT_DISTANCE_THRESHOLD, angle > 60°
        neg_df = df.filter(
        ...
        # Take roughly same number of negatives as positives (to balance)
        ...
        neg_sampled = neg_df.sample(withReplacement=False, fraction=sample_frac, seed=42)

        train_df = pos_df.union(neg_sampled)
        train_df.cache()

        # Feature columns (same for shot)
        feature_cols = ["dist_pb", "player_speed", "ball_speed", "dist_ball_goal", "angle_ball_goal"]
        assembler = VectorAssembler(inputCols=feature_cols, outputCol="features")
        scaler = StandardScaler(inputCol="features", outputCol="scaled_features",
                                withStd=True, withMean=True)
    ...
    # --------------------------------------------------------------------
    # Fallback rule‑based shot detection (if no goals or model fails)
    # --------------------------------------------------------------------
    ...
    def _compute_possession_time(self, df: DataFrame) -> DataFrame:
        ...
        possession_groups = possession_groups.filter(col("duration") >= self.MIN_POSSESSION_SECONDS)
        result = possession_groups.groupBy("player_id").agg(spark_sum("duration").alias("possession_time"))
        return result

    def _aggregate_shots(self, df: DataFrame) -> DataFrame:
        return df.filter(col("shot_pred") == 1) \
                 .groupBy("player_id") \
                 .agg(count("*").alias("shots"))

    # --------------------------------------------------------------------
    # Goal assignment (using possession_pred)
    # --------------------------------------------------------------------
    ...
    def _assign_goals_to_players(self, df: DataFrame, goals_df: DataFrame) -> Dict[int, int]:
        ...
        # Use possession_pred to find who had the ball before each goal
        ...
        joined = ball_players.join(broadcast(goals), how="inner") \
                             .withColumn("time_diff", col("goal_ts") - col("timestamp")) \
                             .filter((col("time_diff") >= 0) & (col("time_diff") <= self.GOAL_ASSOCIATION_WINDOW))
    ...
    def _load_player_states(self, match_id: int, session: Session) -> DataFrame:
        ...
        data = [
            (
                pm.id, pm.track_id,
                ps.frame_number, ps.timestamp,
                1 if ps.has_ball else 0,
                ps.ball_x, ps.ball_y,
                ps.x1, ps.y1, ps.x2, ps.y2,
                ps.dx_meters, ps.dy_meters,
                ps.vx, ps.vy
            )
            for ps, pm in rows
        ]
    ...
    def _load_ball_states(self, match_id: int, session: Session) -> DataFrame:
        stmt = select(BallState).where(BallState.match_id == match_id).order_by(modelcol(BallState.timestamp))
        rows = session.exec(stmt).all()
        if not rows:
            return self.spark.createDataFrame([], self._ball_state_schema())
        data = [
            (bs.frame_number, bs.timestamp, bs.x1, bs.y1, bs.x2, bs.y2,
             bs.dx_meters, bs.dy_meters, bs.confidence)
            for bs in rows
        ]
    ...
    def _build_feature_matrix(self, player_df: DataFrame, ball_df: DataFrame) -> DataFrame:
        # Player centroid
        player_df = player_df.withColumn("px", (col("x1") + col("x2")) / 2) \
                             .withColumn("py", (col("y1") + col("y2")) / 2)
        # Ball centroid
        ball_df = ball_df.withColumn("bx", (col("x1") + col("x2")) / 2) \
                         .withColumn("by", (col("y1") + col("y2")) / 2)

        joined = player_df.join(ball_df, on="frame_number", how="left") \
                          .fillna({"bx": 0.0, "by": 0.0, "dx_meters": 0.0, "dy_meters": 0.0})

        joined = joined.withColumn("dist_pb", sqrt(pow(col("px") - col("bx"), 2) + pow(col("py") - col("by"), 2)))
        joined = joined.withColumn("player_speed", sqrt(pow(col("vx"), 2) + pow(col("vy"), 2)))
        joined = joined.withColumn("ball_speed", sqrt(pow(col("dx_meters"), 2) + pow(col("dy_meters"), 2)))
        return joined

    def _add_goal_features(self, df: DataFrame, centroid: Tuple[float, float]) -> DataFrame:
        gx, gy = centroid
        df = df.withColumn("dist_ball_goal", sqrt(pow(col("bx") - lit(gx), 2) + pow(col("by") - lit(gy), 2)))
        df = df.withColumn(
            "angle_ball_goal",
            degrees(
                acos(
                    when(
                        ...
                        (col("dx_meters") * (lit(gx) - col("bx")) + col("dy_meters") * (lit(gy) - col("by"))) /
    ...
    def _compute_goal_centroid(self, goals_df: DataFrame) -> Optional[Tuple[float, float]]:
        centroids = goals_df.withColumn("cx", (col("x1") + col("x2")) / 2) \
                            .withColumn("cy", (col("y1") + col("y2")) / 2)
        avg_row = centroids.select(avg("cx").alias("cx"), avg("cy").alias("cy")).collect()[0]
        if avg_row.cx is not None and avg_row.cy is not None:
            return (float(avg_row.cx), float(avg_row.cy))
        return None

    def _update_players(self, match_id: int, possession_df: DataFrame,
                        shot_df: DataFrame, goal_assignments: Dict[int, int],
                        session: Session) -> None:
        possession_dict = {row.player_id: row.possession_time for row in possession_df.collect()}
        ...
        for player in players:
            if player.id in possession_dict:
                player.ball_possession_time = possession_dict[player.id]
            if player.id in shot_dict:
                if hasattr(player, "shots"):
                    player.shots = shot_dict[player.id]
            if player.id in goal_assignments:
                player.goals = goal_assignments[player.id]
        session.commit()
