...
class PhysicsCalculatorBase:
    def calculate_distance(self, xo: np.ndarray, xf: np.ndarray) -> Tuple[float, np.ndarray]:
        delta_x = np.array([xo[0] - xf[0], xo[1] - xf[1]])
        return math.fabs(np.linalg.norm(delta_x)), delta_x

    def calculate_speed(self, delta_x: np.ndarray, delta_t: float) -> Tuple[np.ndarray, np.ndarray, float, float]:
        vo = delta_x / delta_t
    ...
    def get_players(self, match_id: int, session: Session) -> Dict[int, List[PlayerState]]:
        players = PlayerRepository.get_players_by_match_id(match_id, session)
        states: Dict[int, List[PlayerState]] = {}

        if len(players) == 0:
            logfire.error(f"[PhysicsCalculator] No players found for match {match_id} in database")
            return states
